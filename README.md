...
- Задача разложена на 7 воспроизводимых шагов → `SKILL.md:37,42,46,57,70,82,88`
- Жёсткий каркас текста: вводка → 5–7 пунктов `### Заголовок → TL;DR → ссылка` → вывод → `SKILL.md:70-74` + `templates/digest.md`
- Формат нарезан под Telegram: подпись ≤1024, сообщения ≤4096, разбивка по `###` → `SKILL.md:84`

**Тон**
- Тон зафиксирован: нейтрально-технический, русский → `SKILL.md:80`
- Вводку/вывод писать самостоятельно, не копируя фрагменты статей → `SKILL.md:78`

**Фактологичность (anti-hallucination)**
- Даты — только из `since`/`until` инструмента, запрет вычислять «сегодня» → `SKILL.md:76`, источник `HabrService.php:73,113`
- TL;DR только по тексту/сниппету, без выдуманных фактов → `SKILL.md:60-61`
- «Никогда не выдумывай ID» → `SKILL.md:53`
- «Фактологичность обязательна» как явное требование → `SKILL.md:80`
...
- Обработка ошибок прямо в инструкции: `error` → пропусти статью, не прерывай дайджест → `SKILL.md:66`, поле `error` от сервера `HabrService.php:128-132`
- «Новых материалов нет» — только при нуле кандидатов (защита от ложного «пусто») → `SKILL.md:55`
