...
что в примере `docs/examples/`; реальный прогон: модель → skill → MCP-инструменты → память,
вывод в stdout):
...
- Команда поднимет зависимости (`mcp-news`, `seeder`); навык `news-digest` подхватывается
  по описанию автоматически (явно — `--skills news-digest`). Память/дедуп обновляются как при
  обычном прогоне.
