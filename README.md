...
**Дедупликация**
- Фильтрация строго механически, по буквальному совпадению ID → `SKILL.md:50-53`
- `SHOWN_IDS` — единственный источник правды (тело дайджеста = память) → `SKILL.md:68`
- Запись в память безусловна, независимо от доставки → `SKILL.md:89`
- Скользящее окно ~50 ID под лимит памяти ~2200 символов → `SKILL.md:92`
