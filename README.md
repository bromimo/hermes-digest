...
docker compose: bridge-сеть appnet
  mcpnews       — PHP MCP-сервер, StreamableHTTP http://mcpnews:8000/mcp (внутри appnet)
...
| **L3** | Дедупликация между запросами через встроенную память Hermes (`MEMORY.md`); обложка дайджеста — `og:image` ведущей статьи (без image-gen) |
...
- **Дедупликация на free-модели стохастична.** Дедуп реализован промптом skill
  через память Hermes (ТЗ L3): модель сама фильтрует кандидатов по показанным ранее
  ID и записывает новые показанные ID обратно. На слабых бесплатных моделях эта
  бухгалтерия иногда сбоит — модель может объявить «новых нет», хотя свежие статьи
  есть (чрезмерное исключение), либо записать в память ID, который не показывала.
  Промпт усилен (единый список `SHOWN_IDS` как источник правды, строго механическая
  фильтрация по буквальным ID, обязательная запись независимо от доставки) — это
  снижает частоту сбоев, но не устраняет их полностью: остаётся **принятым риском
  free-уровня (ТЗ §12)**. Надёжная альтернатива — серверный дедуп с состоянием на
  стороне MCP — сознательно отвергнута, так как противоречит требованию L3 «через
  память Hermes» и принципу stateless-MCP. Для стабильного дедупа используйте более
  сильную (платную) модель через `HERMES_MODEL`.
